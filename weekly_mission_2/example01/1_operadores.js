...
//Ejemplo 3: forEach para imprimir los países en mayúsculas
...
//Ejemplo 5: Uso de Map para convertir todos los nombres de una lista a minúscula
const names = ['Asabeneh', 'Mathias', 'Elias', 'Brook'];
const namesToUpperCase = names.map((name) => name.toUpperCase());

console.log("Ejemplo 5: Uso de Map para convertir todos los nombres de una lista a mayúscula");
console.log(namesToUpperCase);




//Ejemplo 6: Uso de map para convertir todos los nombres de una lista a minúsculas
const countries6 = ['Finland', 'Denmark', 'Sweden', 'Norway', 'Iceland'];
const countriesFirstThreeLetters = countries6.map((country) => country.toLowerCase().slice(0, 3)); //El método slice obtiene solo la longitud marcada del string
...
const countriesContainingLang = countries7.filter((country) => country.includes('land'));
console.log("Ejemplo 7: Uso de filter para filtrar una lista de elementos");
console.log(countriesContainingLang);
...
const result_of_reduce = numbers9.reduce((acc, element) => acc + element, 0); //.reduce(({variable_acumuladora}, {apuntador}) => {operacion acumulable}, {valor_inicial_acumulador});
...
//Ejemplo 14: Uso de some, este método validará todos los elementos de la lista, y si alguno cumple con la validación que indiques.
